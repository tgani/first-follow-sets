...
struct Scanner {
    ...
    Tok la_; // look ahead
    unsigned long long pos_; // read pos in line
...
// Type representing a clause such as { "+" Term } or [ "-" Expression ].
...
public:
    ...
    ProductionItemSeqClause(Tok t)
        : clause_kind_ { t }
    {
        assert(t == Tok::lbrace || t == Tok::lbrack);
    }
    ProductionItemSeq sequence;
};

// Type representing an element of the production's RHS. A production's RHS is composed of a sequence of these.
struct ProductionItem {
    std::variant<Terminal, NonTerminal, Epsilon, ProductionItemSeqClause> item;
...
public:
    ...
    ProductionItemSeqClause scan_production_item_seq_clause()
    {
        ...
        gettoken();
        seq_clause.sequence = scan_production_item_seq();
        require(seq_clause.kind() == Tok::lbrace ? Tok::rbrace : Tok::rbrack);
        return seq_clause;
    }

    ProductionItem scan_production_item()
    {
        if (next_is(Tok::lbrace) || next_is(Tok::lbrack)) {
    ...
    void parse()
    {
        ...
        //      ProductionItemSeqClause = '{' ProductionItemSeq '}' | '[' ProductionItemSeq ']'
    ...
    static std::string production_alternative_text(const ProductionAlternative& prodalt)
    {
        return production_item_seq_text(prodalt.sequence);
    }
    ...
    static std::string productions_text(const ProductionList& prods)
    {
        ...
        for (auto prod : prods) {
            ...
            buf += ' ';
            bool first = true;
            for (auto& item : prod.alternatives) {
                if (!first) {
                    buf += '|';
                    buf += ' ';
                }
                buf += production_alternative_text(item);
                first = false;
            }
            buf += '\n';
        }
...
std::pair<ProductionItemSeq, std::vector<NonTerminal>> replace_sequence_or_optional_clauses(const ProductionItemSeq& prodseq, const std::vector<unsigned>& locations,
    unsigned name_seq)
{
    if (locations.size() == 0)
        return { {}, {} };
...
void generate_invented_productions(const std::vector<NonTerminal>& nt_list, const std::vector<ProductionItemSeqClause>& seq_clause_list,
...
NormalizedProductionList normalize(const ProductionList& prodlist)
{
    ...
    // can contain a sequence-or-optional-clause, we need to do this iteratively till there are
    ...
    while (something_to_do) {
        ...
        for (auto& normprod : result) {
            ...
            auto [replaced_seq, invented_nonterminals] = replace_sequence_or_optional_clauses(normprod.seq, locations, name_seq);
            if (invented_nonterminals.size() > 0)
                normprod.seq = std::move(replaced_seq);
            generate_invented_productions(invented_nonterminals, clause_items, invented_productions);
...
struct CmpGrammarElement {
    ...
    {
        const std::string& lhs_name = lhs.index() == 0 ? std::get<0>(lhs).name : std::get<1>(lhs).name;
        const std::string& rhs_name = rhs.index() == 0 ? std::get<0>(rhs).name : std::get<1>(rhs).name;
        return lhs_name < rhs_name;
    }
};

// Type representing function from terminal/non-terminals -> set of terminals, used in computing FIRST and FOLLOW
...
struct TopDownParsingSets {
    ...
    void insert_into_set(GrammarElementToTerminalSetMap& mapref, const GrammarElement& gelem, const Terminal& t)
    {
    ...
    TerminalSet& predict_set_for(const NonTerminal& nt)
    {
        return get_set_for(predict_, nt);
    }

    TerminalSet& follow_set_for(const NonTerminal& nt)
    {
        return get_set_for(follow_, nt);
    }
    ...
    std::string predict_set_text()
    {
        return text_for_set("PREDICT", predict_);
    }

    std::string follow_set_text()
    {
        return text_for_set("FOLLOW", follow_);
    }
};
...
void compute_first_sets(const ProductionList& prods)
{
    ...
    printf("%s%s%s", parsing_sets.epsilon_set_text().c_str(),
        parsing_sets.predict_set_text().c_str(), parsing_sets.follow_set_text().c_str());
}
